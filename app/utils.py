from app.comments import SUGGESTION_COMMENTS
...
def analyze_food_nutrition(query: str):
    """
...
NUTRITIONIX_APP_ID = "73726a3f"
NUTRITIONIX_API_KEY = "185cd77a51e56e704f98ce5db35bb819"
...
def analyze_drink_nutrition(query: str) -> Dict[str, float | None]:
    ...
    Returns a dict of nutrient values (excluding volume).
    """
    url = "https://trackapi.nutritionix.com/v2/natural/nutrients"
    headers = {
        "x-app-id": NUTRITIONIX_APP_ID,
    ...
    return {
        "calories_estimate": drink.get("nf_calories"),
        "sugar_g": drink.get("nf_sugars"),
        "sodium_mg": drink.get("nf_sodium"),
        "fat_g": drink.get("nf_total_fat"),
        "protein_g": drink.get("nf_protein"),
        "cholesterol_mg": drink.get("nf_cholesterol"),
        "carbohydrates_g": drink.get("nf_total_carbohydrate"),
    }


# -------------------------
# Recommended Daily Allowances (RDA) for 7 Days
# -------------------------
RDA_VALUES = {
    "calories": 2000 * 7,
    "sugar_g": 50 * 7,
...
# -------------------------
# Recommended Daily Allowances (RDA) for 30 Days
# -------------------------
MONTHLY_RDA_VALUES = {k: v * 4 for k, v in RDA_VALUES.items()}


def calculate_nutrient_percentages(totals: dict) -> dict:
    """
    Calculate the percentage of RDA consumed for each nutrient.
    """
    percentages = {}
    for nutrient, value in totals.items():
        rda = RDA_VALUES.get(nutrient, 1)
        percentages[nutrient] = round((value / rda) * 100, 2)
    return percentages


def calculate_field_distribution_percentages(totals: dict) -> dict:
    """
    Calculate the percentage contribution of each nutrient to the total sum of all nutrients.
    """
...
def generate_nutrient_comments(percentages: dict, suggestion_type: str = "weekly") -> dict:
    """
    Generate personalized comments based on nutrient intake percentages.
    ...
    for nutrient, percent in percentages.items():
        ...
        # Use fallback if nutrient or category is not defined
        comment = (
            SUGGESTION_COMMENTS.get(nutrient, {}).get(category)
