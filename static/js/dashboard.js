...
document.addEventListener('DOMContentLoaded', function () {
  ...
  async function trackDrink(drinkType, amount, unit, intakeTime, mlAmount) {
    try {
      ...
      // First try to get from hidden field if exists
      let userId = document.getElementById('user_id')?.value;

      // If not available in a hidden field, try getting from API
      if (!userId) {
        try {
          const userResponse = await fetch('/drink/current_user', {
            method: 'GET',
            credentials: 'include'
          });
          ...
          if (userResponse.ok) {
            ...
            // If we can't get the user ID, show error and return early
            showNotification('Unable to get user ID. Drink saved locally only.', 'warning');
            return;
          }
        } catch (userError) {
          console.error('Error fetching user ID:', userError);
          showNotification('Error getting user ID. Drink saved locally only.', 'warning');
          return;
        }
      }

      if (!userId) {
        showNotification('User ID not found. Drink saved locally only.', 'warning');
        return;
      }

      // Convert intake_time to full datetime format (YYYY-MM-DDTHH:MM:SS)
      const today = new Date();
      const [hours, minutes] = intakeTime.split(':');
      today.setHours(parseInt(hours), parseInt(minutes), 0, 0);
      const fullIntakeTime = today.toISOString();

      // Ensure amount_ml is present and a number
      const finalMlAmount = Math.round(mlAmount) || 0;

      const drinkId = generateProperUUID(); // Generate a UUID for the drink entry
      console.log('Generated drink ID:', drinkId);

      // Send the drink tracking data to the server
      const response = await fetch('/drink/track', {
        ...
        body: JSON.stringify({
          user_id: userId,
          drink_id: drinkId, // Include the UUID here
          drink_type: drinkType,
          amount: String(amount), // Convert to string as required
          unit: unit,
          intake_time: fullIntakeTime, // Send full ISO datetime string
          amount_ml: finalMlAmount, // Ensure it's included and valid
          timestamp: new Date().toISOString()
        }),
        credentials: 'include'
      });
      ...
      if (response.ok) {
        ...
        try {
          ...
          showNotification(`Failed to track drink: ${errorData.detail || 'Unknown error'}`, 'warning');
        } catch (jsonError) {
          showNotification(`Failed to track drink (${response.status}). Saved locally.`, 'warning');
        }
      }
    } catch (error) {
